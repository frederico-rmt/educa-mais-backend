...
def test_create_discursive_question():
  ...
  id_author=UuidIdentifier(UuidGenerator().generate())
  ...
  question = DiscursiveQuestion(id, id_author, title, prompt, expected_answer, criteria, tags, topic, difficulty, grade_level, created_at)
  assert question.id == id
  assert question.id_author == id_author
...
@pytest.mark.parametrize("field,value,error_message", [
  ...
  ("id_author", None, "id_author must be an UuidIdentifier"),
